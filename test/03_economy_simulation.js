...
contract("EconomySimulation", ([lsAcc, artist, artistAccountant, superHatcher, hatcherSimulator, buyer1, buyerSimulator, lastBuyer, feeRecipient]) => {
  ...
  it('should simulate configured market activity from .env file and print economy state', async () => {
    ...
    for (let buyerIndex = 1; buyerIndex < BUYERS - 1; buyerIndex++) {
      ...
      if (buyerIndex % sellerDelta === 0) {
        ...
        const sellAmount = buyersArtistTokens[sellerIndex].mul(new BN(SELLER_AMOUNT_RATIO * 100)).div(new BN(100));
  ...
  it('should let super hatcher to sell his claimed tokens', async () => {
    ...
    const burnAmountWei = artistTokensBalance.mul(new BN(HATCHER_SELL_RATIO * 100)).div(new BN(100));
    const burnAmountPHT = wei2pht(burnAmountWei);
  ...
  it('should let average hatchers to sell their claimed tokens', async () => {
    ...
    const burnAmountWei = artistTokensBalance.mul(new BN(HATCHER_SELL_RATIO * 100)).div(new BN(100));
    const burnAmountPHTPerHatcher = wei2pht(burnAmountWei.div(new BN(AVERAGE_HATCHERS)));
    const burnAmountPHT = wei2pht(burnAmountWei);
